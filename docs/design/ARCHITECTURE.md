...
Performance: one shared RTDB listener per browser, a one-second moving publish cadence, a five-second stationary heartbeat, seven-second HTTPS timeout, jittered backoff, bounded shared rate-limit token leases, Firestore only on lifecycle changes, and local polyline ETA math. Admin-only `GET /api/health` reports rolling processing, device-queue, network, device-to-server, RTDB-write, and rate-limit decision latency plus shared transaction/retry counters, while public `GET /health` is readiness-only. Physical-route latency and GNSS reliability still require the acceptance runbook.

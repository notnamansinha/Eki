...
Performance: one shared RTDB listener per browser, a one-second moving publish cadence, a five-second stationary heartbeat, seven-second HTTPS timeout, jittered backoff, Firestore only on lifecycle changes, and local polyline ETA math. `/health` reports rolling processing, device-queue, network, device-to-server, and RTDB-write latency percentiles. Physical-route latency and GNSS reliability still require the acceptance runbook.

...
`TelemetryFix` is the only queued object; replacement intentionally keeps latest data rather than building an unbounded history. HTTPS 200/202 updates comparison state. Transport errors, 408/425/429 and 5xx retain the latest sample; bounded `Retry-After` is honored. Permanent HTTP rejection drops that sample and delays the next fresh attempt. GNSS loss sends one `uncertain` sample at the last verified point; it never invents movement. An unprovisioned device publishes nothing and exposes only the protected local configuration portal.

`platformio.ini` pins Espressif32 7.0.1, TinyGPSPlus 1.1.0 and ArduinoJson 7.4.3 and defines native, development, and signed fleet environments. `device_config.h` defines the closed, checksummed NVS contract. `sdkconfig.defaults` enables Secure Boot V2, release-mode flash/NVS encryption, and ROM-download lockdown only for the hybrid Arduino/ESP-IDF fleet build. The ignored RSA-3072 signing key is mandatory, and the fleet runtime halts unless both hardware protections are active.
